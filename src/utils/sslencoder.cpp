...
QByteArray SSLEncoder::encryptRSA(QByteArray data, QByteArray keyArray)
{
   ...
   // rsa=PEM_read_bio_RSAPublicKey(bioKey,&rsa,NULL,NULL);
    ...
    if(rsa==NULL)
    {
        ...
        qDebug() << "RSA ENCODE KEY ERROR: " << errorString;
        BIO_free(errBIO);
        BIO_free(bioKey);
        RSA_free(rsa);
        return QByteArray();
...
//    memcpy(key,GlobalConfigInstance.getSessionKey().toLocal8Bit().data(),32);
    ...
    // data.data();
     ...
     ciphertext_len = encryptAES256 (plaintext, data.size(), key, iv, ciphertext);

     /* Do something useful with the ciphertext here */
//     qDebug() << "Ciphertext is:\n";
     QByteArray result;
     if (toBase64)
        result = QByteArray((char*)ciphertext,ciphertext_len).toBase64();
     else
     ...
     if (toBase64)
        return QUrl::toPercentEncoding(result);
     else
         return result;
}
...
int SSLEncoder::decryptAES256(unsigned char *ciphertext, int ciphertext_len, unsigned char *key,
